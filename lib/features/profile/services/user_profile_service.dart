...
class UserProfileService extends GetxService {
  ...
  void loadProfileData() {
    try {
      firstName.value =
          _storageService.read<String>(StorageService.firstNameKey) ?? '';
      lastName.value =
          _storageService.read<String>(StorageService.lastNameKey) ?? '';
      username.value =
          _storageService.read<String>(StorageService.usernameKey) ?? 'User';
      is_teacher.value =
          _storageService.read<bool>(StorageService.isTeacherKey) ?? false;

      // Load avatar settings with better null handling
      final savedAvatarImage =
          _storageService.read<String>(StorageService.avatarImageKey);
      ...
      final savedColorIndex =
          _storageService.read<int>(StorageService.avatarColorIndexKey);
